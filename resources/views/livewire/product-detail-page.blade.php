          ...
          <div class="w-full px-4 md:w-1/2 ">
            <div class="lg:pl-20">
              <div class="mb-8 [&>ul]:list-disc [&>ul]:ml-4">
                ...
                {!! Str::markdown($product->description) !!} <!-- Esta linea hace que se muestre la descripción del producto en formato markdown -->
                </p>
              </div>
              <div class="w-32 mb-8 ">
              ...
              <div class="flex flex-wrap items-center gap-4">
                <button wire:click='addToCart({{ $product->id }})' class="w-full p-4 bg-blue-500 rounded-md lg:w-2/5 dark:text-gray-200 text-gray-50 hover:bg-blue-600 dark:bg-blue-500 dark:hover:bg-blue-700">
                  <span wire:loading.remove wire:target="addToCart({{ $product->id }})">Añadir al carrito</span><span wire:loading wire:target="addToCart({{ $product->id }})">Añadiendo...</span></button>
              </div>
            </div>
          </div>
        </div>
